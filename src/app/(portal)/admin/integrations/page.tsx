...
import { ApiKeysPanel, type ApiKeyRow } from "@/components/portal/admin/api-keys-panel";
import {
  WebhooksPanel,
  type WebhookRow,
  type DeliveryRow,
...
export default async function IntegrationsPage() {
  ...
  const [{ data: keys }, { data: hooks }, { data: deliveries }] = await Promise.all([
    supabase
      .from("api_keys")
      .select("id, name, key_hint, scopes, created_at, last_used_at, expires_at, revoked_at")
    ...
    supabase
      ...
      .select("id, webhook_id, event, status_code, error, attempted_at, duration_ms")
  ...
  const liveKeys = keyRows.filter(
    (k) => k.revoked_at === null && (k.expires_at === null || k.expires_at.slice(0, 10) > today),
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricTile label="Active webhooks" value={String(activeHooks.length)} />
        <MetricTile
          label="Failed deliveries"
          value={String(failedRecently.length)}
          tone={failedRecently.length > 0 ? "warning" : "good"}
        />
        <MetricTile label="Readable resources" value={String(api.resources.length)} />
      </div>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4">
              <p className="text-sm text-muted-foreground">
                Generated from the API itself, so it cannot drift from what the endpoints actually
                do. Every resource takes <code className="font-mono text-xs">limit</code>,{" "}
                ...
                <table className="w-full min-w-[42rem] text-sm">
                  ...
                  <tbody>
                    {api.resources.map((resource) => (
                      <tr key={resource.name} className="border-b border-border/60 last:border-0">
                        <td className="py-2.5 pr-3 align-top font-mono text-xs">{resource.path}</td>
